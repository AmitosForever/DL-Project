...
# -----------------------------
# Clarity-specific: Ordinal regularization
# -----------------------------
def build_clarity_rank(classes: List[str]) -> torch.Tensor:
    ...
    for c in classes:
        if c in rank_map:
            ranks.append(rank_map[c])
        else:
            ranks.append(100 + sorted(classes).index(c))
...
def tta_logits(model: nn.Module, x: torch.Tensor, mode: str) -> torch.Tensor:
    ...
    for cimg in crops:
        ...
        if mode == "fivecrop_flip":
            lgf = model(torch.flip(cimg, dims=[3]))
            logits_sum = logits_sum + lgf
            n += 1

    return logits_sum / float(n)


@torch.no_grad()
def evaluate(model, loader, device, criterion_hard, tta_mode: str = "none") -> Tuple[Metrics, torch.Tensor]:
    model.eval()
    total_loss = 0.0
    n = 0
...
def train_one_epoch(
    ...
    # Stage B uses hard CE; Stage A uses soft CE
    ...
    for step, (x, y) in pbar:
        ...
        if use_amp:
            with autocast("cuda"):
                ...
                if did_mix:
                    ...
                    loss_main = hard_criterion(logits, y)

                loss = loss_main
                if ordinal_lambda > 0 and class_ranks is not None:
                    loss = loss + (ordinal_lambda * ordinal_expected_rank_loss(logits, y, class_ranks))
        ...
        # scheduler step AFTER optimizer.step (and ours never warns anyway)
        scheduler.step()
        ...
        # batch_acc is meaningful only when NOT using mix
        with torch.no_grad():
            pred = logits.argmax(dim=1)
        ...
        if step % log_every == 0 or step == 1:
            ...
            if did_mix:
                pbar.set_postfix(loss=f"{avg_loss:.4f}", batch_acc="NA(mix)", lr=f"{lr:.2e}", t=f"{elapsed:.0f}s")
            else:
...
def main():
    ...
    ap.add_argument("--img_size", type=int, default=384, help="Stage A img size")
    ap.add_argument("--img_size_ft", type=int, default=512, help="Stage B img size")
    ...
    print(f"[INFO] Progressive resize: stageA={args.img_size} | stageB(ft)={args.img_size_ft}")
    ...
    print(f"[INFO] Ordinal lambda={args.ordinal_lambda}")
    ...
    # Validate at high-res from the start (clarity usually benefits)
    val_tfm = build_transforms(args.img_size_ft, mean, std, train=False)

    ds_train = ClarityDataset(df_train, img_col, label_col, args.root_dir, label2idx, tfm=train_tfm_A,
    ...
    if args.sampler:
        ...
        print("[INFO] Sampler: WeightedRandomSampler ON (Stage A).")
    else:
    ...
    class_ranks = build_clarity_rank(classes).to(device)
    ...
    # Scheduler for Stage A
    steps_per_epoch = len(dl_train)
    total_steps_A = steps_per_epoch * mix_epochs
    warmup_steps_A = steps_per_epoch * max(0, args.warmup_epochs)
    sched = WarmupCosine(optimizer, warmup_steps=warmup_steps_A, total_steps=max(1, total_steps_A))
    sched.set_lr(0)  # initial LR for step=0
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if (not fine_tune_started) and (epoch == mix_epochs + 1):
            ...
            # sampler OFF for stability
            dl_train = make_train_loader(use_sampler=False)

            # train transforms to high-res
            ...
            # rebuild scheduler for Stage B (remaining steps)
            steps_per_epoch = len(dl_train)
            remaining_epochs = max(1, args.epochs - mix_epochs)
            total_steps_B = steps_per_epoch * remaining_epochs
            warmup_steps_B = 0
            sched = WarmupCosine(optimizer, warmup_steps=warmup_steps_B, total_steps=max(1, total_steps_B))
            sched.set_lr(0)

            print(f"[STAGE] Fine-tune starts at epoch {epoch}: sampler OFF, train img_size -> {args.img_size_ft}, lr -> {args.lr_ft}")
        ...
        val_metrics, conf = evaluate(model, dl_val, device, criterion_hard, tta_mode="none")
        if ema is not None:
            ema.restore(model)
    ...
    if ema is not None:
        ema.apply_shadow(model)
    tta_metrics, tta_conf = evaluate(model, dl_val, device, criterion_hard, tta_mode=args.tta_mode)
    if ema is not None:
        ema.restore(model)
