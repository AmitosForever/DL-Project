...
class ClarityNet(nn.Module):
    def __init__(self, num_classes: int, dropout: float = 0.1, task: str = "ordinal_mse"):
        ...
        # -----------------------------
        # UPDATED: stability init for ordinal regression (new midpoint 10.0 on 0..20 scale)
        # -----------------------------
        if self.task == "ordinal_mse":
            try:
                nn.init.constant_(self.backbone.classifier[3].bias, 10.0)
            except Exception:
                final = self.backbone.classifier[-1]
                if isinstance(final, nn.Linear) and final.bias is not None:
                    nn.init.constant_(final.bias, 10.0)
...
def build_clarity_rank(classes: List[str]) -> torch.Tensor:
    # UPDATED: non-linear ordinal scale to increase distances between confusing boundaries.
    rank_map = {
        "FL": 0.0, "IF": 1.0,      # Gap 1
        "VVS1": 2.0, "VVS2": 3.0,  # Gap 1
        "VS1": 4.5, "VS2": 6.0,    # Gap 1.5 (Help separate VS1/VS2)
        "SI1": 8.0,                # Gap 2.0
        "SI2": 10.0,               # Gap 2.0
        "I1": 13.0,                # Gap 3.0 (CRITICAL: Penalize mixing SI2/I1)
        "I2": 16.0,                # Gap 3.0
        "I3": 20.0                 # Gap 4.0
    }
...
def evaluate(
    ...
    for batch in it:
        ...
        if task == "ordinal_mse":
            ...
            # UPDATED: map predicted scalar -> nearest class by rank distance (supports non-integer ranks like 4.5)
            # pred_idx: [B]
...
def train_one_epoch(
    ...
    for step, batch in pbar:
        ...
        if task == "ordinal_mse":
            ...
            if use_amp:
                with autocast_context(autocast_fn, device, enabled=True):
                    ...
                    loss_vec = mse_criterion(pred_rank, y_aug)
        ...
        with torch.no_grad():
            if task == "ordinal_mse":
                # UPDATED: compute class accuracy by nearest-rank assignment (only meaningful when not mixed)
                if did_mix:
                    batch_acc = 0.0
                else:
...
def main():
    ...
    ap.add_argument("--img_size_ft", type=int, default=448, help="Stage B img size (fine-tune)")
    ...
    ap.add_argument("--use_loss_weights", action="store_true", default=True)
    ...
    # ranks aligned with classes (non-linear scale)
    ...
    criterion_hard = nn.MSELoss() if args.task == "ordinal_mse" else nn.CrossEntropyLoss()
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if (not fine_tune_started) and (epoch == mix_epochs + 1):
            ...
            print(f"[STAGE] Fine-tune starts at epoch {epoch}: sampler={'ON' if args.sampler else 'OFF'}, "
