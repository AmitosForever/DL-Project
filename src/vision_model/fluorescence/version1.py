...
# Allows loading partially truncated JPEGs instead of crashing.
...
try:
    ...
    raise RuntimeError("tqdm is required for progress bars: pip install tqdm") from e
...
def normalize_fluorescence_label(x: str) -> Optional[str]:
    ...
    Supports:
      ...
      - "Medium Blue"/etc -> Medium (color word removed)
    """
    if x is None:
        return None
...
class DiamondFluorescenceDataset(Dataset):
    def __init__(self, df: pd.DataFrame, img_col: str, label_idx_col: str, transform=None, max_retry: int = 10):
    ...
    def __getitem__(self, idx: int):
        # Robust: if a file is broken, try another index instead of crashing training.
        n = len(self.df)
        ...
        # If we got here, too many failures in a row: surface which path was last attempted.
...
def clean_and_build_dataframe(
    ...
    if verify_images:
        # Slower but removes broken files up-front.
        ok = []
...
# -----------------------------
def main():
    parser = argparse.ArgumentParser("Diamond Fluorescence Classifier (ConvNeXt-Tiny) - fixed split + robust images")
    ...
    parser.add_argument("--out_dir", type=str, default="_fluor_out", help="Output directory.")
    ...
    parser.add_argument("--use_sampler", action="store_true", help="Use WeightedRandomSampler for imbalance.")
    parser.add_argument("--use_focal", action="store_true", help="Use FocalLoss instead of CrossEntropy.")
    ...
    label_to_idx = {lab: i for i, lab in enumerate(CANONICAL_5)}
    df["_label_idx"] = df[fluor_col_used].apply(lambda v: label_to_idx[normalize_fluorescence_label(v)]).astype(int)

    dist = df[fluor_col_used].value_counts().to_dict()
    print("[INFO] Fluorescence distribution (5-class normalized):")
    for k in CANONICAL_5:
    ...
    for name, part in [("train", df_train), ("val", df_val), ("test", df_test)]:
        d = part[fluor_col_used].value_counts().to_dict()
        print(f"[SPLIT_DIST] {name}: " + ", ".join([f"{k}={int(d.get(k,0))}" for k in CANONICAL_5]))
    ...
    if args.use_focal:
        ...
        print(f"[INFO] Loss=FocalLoss(gamma={args.focal_gamma}) + class weights")
    else:
        criterion = nn.CrossEntropyLoss(weight=class_weights)
        print("[INFO] Loss=CrossEntropyLoss + class weights")
