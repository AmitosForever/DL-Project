...
# Model / Loss
# -----------------------------
def build_convnext_tiny_backbone(pretrained: bool = True, stochastic_depth_prob: float = 0.2) -> nn.Module:
    kwargs = {}
    try:
        # torchvision versions differ; if unsupported, fallback
        kwargs["stochastic_depth_prob"] = float(stochastic_depth_prob)
    except Exception:
        pass
    ...
    if pretrained and _HAS_CONVNEXT_WEIGHTS:
        ...
        try:
            model = convnext_tiny(weights=weights, **kwargs)
        except TypeError:
            model = convnext_tiny(weights=weights)
    else:
        try:
            model = convnext_tiny(weights=None, **kwargs)
        except TypeError:
...
class DualStreamNetwork(nn.Module):
    def __init__(
        ...
        # dropout right before final classification layer (Gemini request)
...
def compute_non_none_precision_recall(y_true: np.ndarray, y_pred: np.ndarray, none_idx: int = 0) -> Tuple[float, float]:
    yt = (y_true != none_idx).astype(int)
    yp = (y_pred != none_idx).astype(int)
...
def train_one_epoch(
    ...
    nn_prec, nn_rec = compute_non_none_precision_recall(y_true, y_pred, none_idx=0)
    avg_loss = total_loss / max(1, total_seen)
    return EpochStats(loss=avg_loss, acc=acc, macro_f1=macro_f1, non_none_prec=nn_prec, non_none_rec=nn_rec)
...
def evaluate(
    ...
    desc: str,
    use_tta: bool,
    prior_log: Optional[torch.Tensor],
    prior_tau: float,
    ...
    for xb, tb, yb in pbar:
        ...
        with autocast_ctx_fn():
            ...
            # Prior correction to counter sampler-induced hallucinations
            if prior_log is not None:
                logits = logits + float(prior_tau) * prior_log
    ...
    nn_prec, nn_rec = compute_non_none_precision_recall(y_true, y_pred, none_idx=0)
    avg_loss = total_loss / max(1, total_seen)
    return EpochStats(loss=avg_loss, acc=acc, macro_f1=macro_f1, non_none_prec=nn_prec, non_none_rec=nn_rec), y_true, y_pred
...
# -----------------------------
# Transforms: STRICT color integrity + heavier spatial aug
# -----------------------------
def build_transforms(img_size: int = 224) -> Tuple[transforms.Compose, transforms.Compose]:
    ...
    train_tf = transforms.Compose([
        ...
        # spatial-only heavy augmentation (Gemini request)
        ...
        # STRICT color integrity: NO hue/saturation jitter at all
...
# -----------------------------
def main():
    parser = argparse.ArgumentParser("Diamond Fluorescence (Multi-Modal ConvNeXt + Tabular) - v5")
    ...
    parser.add_argument("--out_dir", type=str, default="_fluor_out_v5", help="Output directory.")
    ...
    parser.add_argument("--weight_decay", type=float, default=0.1)  # Gemini: 0.1
    ...
    parser.add_argument(
        ...
        default="auto",
        choices=["auto", "on", "off"],
        help="auto=OFF when sampler ON, ON when sampler OFF.",
    )

    parser.add_argument("--warmup_epochs", type=int, default=2, help="Epochs to train head-only (freeze backbone).")
    ...
    # Prior correction (critical when sampler is ON)
    parser.add_argument("--prior_correction", type=str, default="auto", choices=["auto", "on", "off"])
    parser.add_argument("--prior_tau", type=float, default=1.0)

    parser.add_argument("--stochastic_depth", type=float, default=0.2, help="ConvNeXt stochastic depth prob (if supported).")
    ...
    df, fluor_col_used, img_col_used = clean_and_build_dataframe(csv_path, data_root, fluor_col, img_col)
    ...
    # Prior log (estimated on TRAIN split actual distribution)
    num_classes = len(labels)
    train_labels_idx = df_train["_label_idx"].values.astype(int)
    prior = np.bincount(train_labels_idx, minlength=num_classes).astype(np.float64)
    prior = prior / max(1.0, prior.sum())
    prior_log = torch.tensor(np.log(prior + 1e-12), dtype=torch.float32)

    # Prior correction auto: ON if sampler ON, else OFF
    if args.prior_correction == "on":
        use_prior = True
    elif args.prior_correction == "off":
        use_prior = False
    else:
        use_prior = bool(use_sampler)
    ...
    backbone = build_convnext_tiny_backbone(pretrained=True, stochastic_depth_prob=args.stochastic_depth)
    # dropout set high to fight overfit (Gemini: 0.5 before final layer)
    ...
    prior_log_t = prior_log.to(device) if use_prior else None
    if use_prior:
        print(f"[INFO] Prior correction: ON (tau={args.prior_tau})")
    else:
        print("[INFO] Prior correction: OFF")
    ...
    meta = {
        ...
        "bg_rgb": list(bg_rgb),
        "tta": bool(args.tta),
        "prior_correction": ("on" if use_prior else "off"),
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(
            ...
            f"(nonNone P/R={train_stats.non_none_prec:.3f}/{train_stats.non_none_rec:.3f}) | "
            f"val: loss={val_stats.loss:.4f} acc={val_stats.acc:.4f} f1={val_stats.macro_f1:.4f} "
            f"(nonNone P/R={val_stats.non_none_prec:.3f}/{val_stats.non_none_rec:.3f}) | "
            f"time={dt:.1f}s"
        )
    ...
    print(f"[TEST] loss={test_stats.loss:.4f} acc={test_stats.acc:.4f} macro-F1={test_stats.macro_f1:.4f} "
          f"(nonNone P/R={test_stats.non_none_prec:.3f}/{test_stats.non_none_rec:.3f})")
    ...
    report = {
        ...
        "test_non_none_precision": test_stats.non_none_prec,
        "test_non_none_recall": test_stats.non_none_rec,
