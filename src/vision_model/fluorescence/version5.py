...
def build_label_space_forced_binary() -> Tuple[List[str], Dict[str, str]]:
...
class DualStreamNetwork(nn.Module):
    def __init__(
        ...
        # Heavy dropout right before classifier to fight overfit
...
# -----------------------------
# Metrics / Prediction logic (Thresholding for binary)
# -----------------------------
def logits_to_pred_binary(logits: torch.Tensor, thr: float) -> torch.Tensor:
    probs = torch.softmax(logits, dim=1)
    pred = (probs[:, 1] > float(thr)).long()
    return pred
...
def compute_pos_precision_recall(y_true: np.ndarray, y_pred: np.ndarray) -> Tuple[float, float]:
...
def train_one_epoch(
    ...
    for xb, tb, yb in pbar:
        ...
        pred = logits_to_pred_binary(logits.detach(), thr=thr)
    ...
    acc, macro_f1 = compute_metrics_binary(y_true, y_pred)
    pos_prec, pos_rec = compute_pos_precision_recall(y_true, y_pred)
    avg_loss = total_loss / max(1, total_seen)
    return EpochStats(loss=avg_loss, acc=acc, macro_f1=macro_f1, pos_prec=pos_prec, pos_rec=pos_rec)
...
def evaluate(
    ...
    for xb, tb, yb in pbar:
        ...
        with autocast_ctx_fn():
            ...
            # PRIOR CORRECTION: REMOVED (as requested)
            loss = criterion(logits, yb)
        ...
        pred = logits_to_pred_binary(logits, thr=thr)
    ...
    pos_prec, pos_rec = compute_pos_precision_recall(y_true, y_pred)
    avg_loss = total_loss / max(1, total_seen)
    return EpochStats(loss=avg_loss, acc=acc, macro_f1=macro_f1, pos_prec=pos_prec, pos_rec=pos_rec), y_true, y_pred
...
# -----------------------------
# Transforms: STRICT color integrity + spatial aug
# -----------------------------
...
# -----------------------------
def main():
    parser = argparse.ArgumentParser("Diamond Fluorescence (Binary + Threshold) - v6")
    ...
    parser.add_argument("--out_dir", type=str, default="_fluor_out_v6", help="Output directory.")
    ...
    # Forced binary regardless
    parser.add_argument("--label_mode", type=str, default="binary", help="Ignored: binary is forced.")
    ...
    parser.add_argument("--thr", type=float, default=0.35, help="Binary threshold for class 1 (fluorescent).")
    ...
    labels, collapse = build_label_space_forced_binary()
    ...
    print(f"[INFO] label_mode={args.label_mode} | num_classes={len(labels)} | labels={labels} | thr={args.thr}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(
            ...
            f"(pos P/R={train_stats.pos_prec:.3f}/{train_stats.pos_rec:.3f}) | "
            f"val: loss={val_stats.loss:.4f} acc={val_stats.acc:.4f} f1={val_stats.macro_f1:.4f} "
            f"(pos P/R={val_stats.pos_prec:.3f}/{val_stats.pos_rec:.3f}) | "
            f"time={dt:.1f}s"
        )
    ...
    print(f"[TEST] loss={test_stats.loss:.4f} acc={test_stats.acc:.4f} macro-F1={test_stats.macro_f1:.4f} "
          f"(pos P/R={test_stats.pos_prec:.3f}/{test_stats.pos_rec:.3f})")
    ...
    report = {
        ...
        "test_pos_precision": test_stats.pos_prec,
        "test_pos_recall": test_stats.pos_rec,
