...
def preprocess_crops(
    ...
    for i in pbar:
        ...
        try:
            ...
            bad += 1
            # keep absolute original so later cache-root won't break this row
...
def main():
    ...
    ap.add_argument("--out", default="cut_best.pt")
    ...
    ap.add_argument("--cache_dir", default="_cut_cache_crops")
    ...
    label_col = args.label_col or guess_column(cols, ["cut"])
    ...
    ds_train = CutDataset(df_train, img_col, label_col, args.root_dir, label2idx, image_index, tfm=tfm_train, max_decode_retries=args.max_decode_retries)
    ds_val = CutDataset(df_val, img_col, label_col, args.root_dir, label2idx, image_index, tfm=tfm_val, max_decode_retries=args.max_decode_retries)
    ...
    model = CutNet(num_classes=num_classes, dropout=args.dropout).to(device)
    if device.type == "cuda":
    ...
    for epoch in range(1, args.epochs + 1):
        # Fine-tune stage: bigger res + smaller LR (because reality hates optimism)
