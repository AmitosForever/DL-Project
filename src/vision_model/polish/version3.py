...
# -----------------------------
# Focal Cross Entropy (hard labels)
# -----------------------------
class FocalCE(nn.Module):
    def __init__(self, gamma: float = 1.0, weight: Optional[torch.Tensor] = None, label_smoothing: float = 0.0):
        super().__init__()
...
# -----------------------------
# Logit Adjustment (for imbalance)
# logits_adj = logits + tau * log(prior)
# Note: log(prior) is negative for small priors, so this *penalizes* rare classes.
# In this script we apply it in a controlled schedule and DISABLE in fine-tune.
# -----------------------------
def logit_adjustment(logits: torch.Tensor, log_priors: torch.Tensor, tau: float) -> torch.Tensor:
    if tau <= 0:
        return logits
    return logits + (tau * log_priors.view(1, -1).to(logits.device, logits.dtype))
...
def build_transforms(img_size: int, mean, std, train: bool, random_erasing_p: float = 0.0):
    if train:
        ...
        if random_erasing_p > 0:
            tfms.append(transforms.RandomErasing(p=float(random_erasing_p), scale=(0.02, 0.12), ratio=(0.3, 3.3), value="random"))
...
def print_confusion_matrix(conf: torch.Tensor, classes: List[str], normalize: Optional[str] = None, title: str = "[VAL] Confusion Matrix"):
    """
    normalize:
      None -> raw counts
      'true' -> rows sum to 1 (per-true-class)
      'pred' -> cols sum to 1 (per-pred-class)
      'all' -> divide by total
    """
    cm = conf.detach().cpu().float()
    if normalize is not None:
        norm = normalize.lower()
        if norm == "true":
            cm = cm / cm.sum(dim=1, keepdim=True).clamp_min(1e-12)
        elif norm == "pred":
            cm = cm / cm.sum(dim=0, keepdim=True).clamp_min(1e-12)
        elif norm == "all":
            cm = cm / cm.sum().clamp_min(1e-12)

    w = max(6, max(len(c) for c in classes) + 2)
    print(title + (f" (normalize={normalize})" if normalize else ""))
    header = " " * w + "".join([f"{c:>{w}}" for c in classes])
    print(header)
    for i, c in enumerate(classes):
        row = "".join([f"{cm[i, j].item():>{w}.4f}" if normalize else f"{int(cm[i, j].item()):>{w}d}" for j in range(len(classes))])
        print(f"{c:<{w}}{row}")
...
# -----------------------------
def effective_num_weights(counts: List[int], beta: float = 0.9999) -> List[float]:
    # weights_i ∝ (1 - beta) / (1 - beta^n_i)
    w = []
    for n in counts:
        n = max(1, int(n))
...
def train_one_epoch(
    ...
    log_priors: Optional[torch.Tensor],
    tau: float,
):
    model.train()

    # Hard loss (only used when no mix)
    if focal_gamma > 0:
        hard_loss_fn = FocalCE(gamma=focal_gamma, weight=class_weight_tensor, label_smoothing=label_smoothing)
    else:
        hard_loss_fn = nn.CrossEntropyLoss(weight=class_weight_tensor, label_smoothing=max(0.0, float(label_smoothing)))
    ...
    for step, (x, y) in pbar:
        ...
        with autocast_context(autocast_fn, device, enabled=use_amp):
            ...
            # controlled logit adjustment
            if (log_priors is not None) and (tau > 0):
                logits = logit_adjustment(logits, log_priors=log_priors, tau=tau)
        ...
        pbar.set_postfix(loss=f"{avg_loss:.4f}", acc=f"{batch_acc*100:.2f}%", lr=f"{lr_now:.2e}", tau=f"{tau:.2f}", upd=updates, accum=accum_steps)
...
def evaluate(
    ...
    for x, y in pbar:
        ...
        if tta:
            logits = tta_predict_logits(model, x)
        else:
            logits = model(x)
...
# -----------------------------
def build_optimizer_with_head_lr(model: nn.Module, base_lr: float, wd: float, head_lr_mult: float):
    # ConvNeXt head is classifier
    head_params = []
    base_params = []
    for name, p in model.named_parameters():
    ...
    param_groups = [
        ...
        {"params": head_params, "lr": base_lr * float(head_lr_mult), "weight_decay": wd},
    ]
    return torch.optim.AdamW(param_groups)


def set_head_lr_mult(optimizer: torch.optim.Optimizer, base_lr: float, head_lr_mult: float):
    if len(optimizer.param_groups) >= 2:
        optimizer.param_groups[0]["lr"] = base_lr
        optimizer.param_groups[1]["lr"] = base_lr * float(head_lr_mult)
    else:
...
# Tau schedule
# -----------------------------
def tau_schedule(epoch: int, start: float, end: float, warmup_epochs: int) -> float:
    if warmup_epochs <= 0:
        return float(end)
    if epoch <= warmup_epochs:
        return float(start)
    # linear decay from warmup+1 to end of training handled in main with epochs
    return float(start)  # placeholder, actual interpolation done in main
...
def main():
    ...
    ap.add_argument("--out_bal", default="polish_best_bal.pt")
    ap.add_argument("--out_f1", default="polish_best_f1.pt")
    ...
    # Fine-tune aug (prevent collapse)
    ap.add_argument("--mix_p_ft", type=float, default=0.15)
    ap.add_argument("--mixup_alpha_ft", type=float, default=0.10)
    ap.add_argument("--cutmix_alpha_ft", type=float, default=0.0)
    ...
    ap.add_argument("--effective_beta", type=float, default=0.9999)
    ...
    ap.add_argument("--disable_tau_in_ft", action="store_true", default=True)
    ap.add_argument("--disable_weights_in_ft", action="store_true", default=True)

    # focal (only when no mix)
    ap.add_argument("--focal_gamma", type=float, default=1.0)

    # head lr mult
    ap.add_argument("--head_lr_mult", type=float, default=3.0)
    ap.add_argument("--head_lr_mult_ft", type=float, default=1.5)
    ...
    ap.add_argument("--cm_normalize", default=None, choices=[None, "true", "pred", "all", "None"])
    ...
    # Image index (for resolving filenames robustly)
    ...
    tfm_val = build_transforms(args.img_size_ft, mean, std, train=False, random_erasing_p=0.0)
    ...
    optimizer = build_optimizer_with_head_lr(model, base_lr=args.lr, wd=args.wd, head_lr_mult=args.head_lr_mult)
    ...
    # Scheduler: per optimizer update (not per micro-batch)
    ...
    # Class priors from TRAIN distribution
    train_counts = [0] * num_classes
    for y in df_train[label_col].tolist():
        train_counts[label2idx[y]] += 1
    priors = torch.tensor(train_counts, dtype=torch.float32)
    priors = (priors / priors.sum().clamp_min(1.0)).clamp_min(1e-12)
    log_priors = torch.log(priors)

    # Effective-num class weights
    class_weights = None
    if args.use_class_weights:
        w = effective_num_weights(train_counts, beta=args.effective_beta)
        class_weights = torch.tensor(w, dtype=torch.float32, device=device)
        print(f"[INFO] Class weights (effective-num, beta={args.effective_beta}): {w}")
    else:
        print("[INFO] Class weights: OFF")

    print(f"[INFO] Class priors: {priors.tolist()}")
    ...
    def tau_for_epoch(epoch: int) -> float:
        # warmup: fixed start for first tau_warmup epochs
        if args.tau_warmup <= 0:
            return float(args.tau_end)
        if epoch <= args.tau_warmup:
            return float(args.tau_start)
        # linearly decay from (tau_warmup+1) to epochs
        t0 = float(args.tau_start)
        t1 = float(args.tau_end)
        denom = max(1, (args.epochs - args.tau_warmup))
        k = (epoch - args.tau_warmup) / float(denom)
        k = min(max(k, 0.0), 1.0)
        return t0 + (t1 - t0) * k

    for epoch in range(1, args.epochs + 1):
        in_ft = (epoch >= args.ft_from_epoch)

        # Fine-tune stage: bigger res + smaller LR
        if epoch == args.ft_from_epoch:
            print(f"[STAGE] Fine-tune from epoch {epoch}: img_size=512, lr_ft={args.lr_ft}")
            ds_train.tfm = build_transforms(args.img_size_ft, mean, std, train=True, random_erasing_p=args.random_erasing)

            # Update LR and head multiplier for FT
            set_head_lr_mult(optimizer, base_lr=args.lr_ft, head_lr_mult=args.head_lr_mult_ft)

        # Tau (disable in FT to prevent collapse)
        tau = tau_for_epoch(epoch)
        if in_ft and args.disable_tau_in_ft:
            tau = 0.0

        # Disable class weights in FT (keep sampler) to avoid double-imbalance hacks
        cw = class_weights
        if in_ft and args.disable_weights_in_ft:
            cw = None

        mix_p = args.mix_p if not in_ft else args.mix_p_ft
        mixup_a = args.mixup_alpha if not in_ft else args.mixup_alpha_ft
        cutmix_a = args.cutmix_alpha if not in_ft else args.cutmix_alpha_ft
        ...
        train_loss = train_one_epoch(
            ...
            mix_p=mix_p,
            mixup_alpha=mixup_a,
            cutmix_alpha=cutmix_a,
            grad_clip=args.grad_clip,
            accum_steps=args.accum_steps,
            focal_gamma=args.focal_gamma,
            class_weight_tensor=cw,
        ...
        val_metrics, conf = evaluate(
            ...
            desc="val",
            tta=args.tta,
            log_priors=(log_priors if tau > 0 else None),
            tau=tau,
        )
        ...
        if args.print_cm:
            print_confusion_matrix(conf, classes, normalize=None, title=f"[VAL] Confusion Matrix @ epoch {epoch:02d}")
            if args.cm_normalize is not None:
                print_confusion_matrix(conf, classes, normalize=args.cm_normalize, title=f"[VAL] Confusion Matrix @ epoch {epoch:02d}")
        ...
        if val_metrics.bal_acc > best_bal:
            ...
            torch.save(ckpt, args.out_bal)
            torch.save(ckpt, args.out)  # keep backward compatibility with your old filename
            print(f"[SAVE] best -> {args.out} (val_bal_acc={best_bal*100:.2f}%)")

        # Save best by macroF1 (often more stable with tiny GD)
        if val_metrics.macro_f1 > best_f1:
            ...
            ckpt = {
                ...
                "weights": "ConvNeXt_Tiny_Weights.DEFAULT",
                "task": "polish_classification",
                "best_metric": "val_macroF1",
                "best_value": best_f1,
                "train": vars(args),
            }
            torch.save(ckpt, args.out_f1)
            print(f"[SAVE] best_f1 -> {args.out_f1} (val_macroF1={best_f1:.4f})")

    print(f"[DONE] best_val_bal_acc={best_bal*100:.2f}% | best_val_macroF1={best_f1:.4f} | saved={args.out}, {args.out_bal}, {args.out_f1}")
