...
def soft_cross_entropy_weighted(
    ...
    """
    Weighted soft-label cross entropy for MixUp/CutMix.
    If class_weights is provided, it weights classes.
    We normalize per-sample by weighted target mass to keep scale stable.
    """
    logp = F.log_softmax(logits, dim=1)  # [B,C]
    if class_weights is None:
        return -(y_soft * logp).sum(dim=1).mean()

    w = class_weights.view(1, -1).to(logits.device)  # [1,C]
    num = -(y_soft * logp * w).sum(dim=1)  # [B]
    den = (y_soft * w).sum(dim=1).clamp_min(1e-12)  # [B]
    return (num / den).mean()


# -----------------------------
# Fairness helpers: Class-balanced weights + Logit adjustment
# -----------------------------
def compute_class_weights_effective_num(
    ...
    weights_c = (1-beta)/(1-beta^n_c), normalized to mean=1
    """
...
def logit_adjustment(logits: torch.Tensor, class_priors: torch.Tensor, tau: float = 1.0) -> torch.Tensor:
    # Adds tau * log(p(y)) to logits, discouraging majority-class bias
...
def train_one_epoch(
    ...
    if class_weights is not None:
        ...
        ce_hard = nn.CrossEntropyLoss(label_smoothing=max(0.0, float(label_smoothing)))
    ...
    for step, (x, y) in pbar:
        ...
        with autocast_context(autocast_fn, device, enabled=use_amp):
            ...
            if (class_priors is not None) and (logit_tau != 0.0):
                logits = logit_adjustment(logits, class_priors, tau=logit_tau)
...
def main():
    ...
    # NEW: fairness knobs
    ap.add_argument("--cb_beta", type=float, default=0.9999, help="Class-balanced beta for effective number weights")
    ap.add_argument("--logit_tau", type=float, default=1.0, help="Logit adjustment strength (0 disables)")
    ...
    # NEW: compute fairness stats from TRAIN labels
    ...
    print(f"[INFO] Logit adjustment tau={args.logit_tau}")
